...
import { CartRes, Product } from './../../../interfaces/cart.interface';
...
export default function Cart() {
    ...
    const {mutate,isPending} = useMutation({mutationFn:clearCart,
        onSuccess:(data)=>{
...
function ProductItemTable({prod}:{prod:Product}){
    ...
    const {mutate:updateMutate,isPending:updatePendion} = useMutation({mutationFn:updateCount,
        onSuccess:(data)=>{
            toast.success(data?.message)
            queryClient.invalidateQueries({queryKey:['cart']})
        },
  ...
  prod.count < prod.product.quantity && updateMutate({
        ...
        <td className="px-6 py-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <span  id="first_product" className="bg-gray-50 w-14 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" >
                {updatePendion?<i className='fa-solid fa-spin fa-spinner'></i>: prod.count}
              </span>
